<?php
/**
 * Database object and functions
 * 
 * @version $Id: database.php,v 1.9 2006/06/14 23:36:44 gconklin Exp $
 * @package Intranet
...
class database {
	...
	*/
	function database( $host='localhost', $user, $pass, $db, $table_prefix ) {
		...
		if (!function_exists( 'mysql_connect' )) {
			...
			$systemError = 'FATAL ERROR: MySQL support not available.  Please check your configuration.';
			$basePath = dirname( __FILE__ );
			#include $basePath . '/includes/configuration.php';
		...
		if (!($this->_resource = @mysql_connect( $host, $user, $pass ))) {
			...
			$systemError = 'FATAL ERROR: Connection to database server failed.';
			$basePath = dirname( __FILE__ );
			#include $basePath . '/includes/configuration.php';
			include 'offline.inc.php';
			exit();
		...
		if (!mysql_select_db($db)) {
			...
			$systemError = "FATAL ERROR: Database not found. Operation failed with error: ". mysql_error();
			$basePath = dirname( __FILE__ );
			#include $basePath . '/includes/configuration.php';
			include 'offline.inc.php';
			exit();
	...
	*/
	function query() {
		#global $CONFIG_debug;		# don't think we need this
		if ($this->_debug) {
			$this->_ticker++;
	  		$this->_log[] = $this->_sql;
		}
	...
	function query_batch( $abort_on_error=true, $p_transaction_safe = false) {
		...
		# gconklin: added \n to search string which forces batch jobs to have new lines 
		#  but allows semicolons in values
		$query_split = preg_split ("/[;]\n+/", $this->_sql);
		$error = 0;
	...
	* @return true if row was returned
	*/
	...
	* @param object $object reference to object with filled in values
	* @param string $keyName name of table column to identify in WHERE specification
	* @param boolean $updateNulls specify if null values should be updated to null or bybassed
	* @param boolean $verbose verbose response if true
	...
	*
	* Updates a record using an object with filled in values.
	*
	* @param string $table table name
	* @param object $object reference to object with filled in values
	* @param string $keyName name of table column to identify in WHERE specification
	* @param boolean $updateNulls specify if null values should be updated to null or bypassed
	* @return true if successful otherwise returns an error message
	*/
	function updateObject( $table, &$object, $keyName, $updateNulls=true ) {
		...
		foreach (get_object_vars( $object ) as $k => $v) {
			...
			if ( preg_match("/^now\(\)$/", $v) ) {
				...
				$val = "''";
				# if table column doesn't allow null, this would bomb:
				#$val = 'null';
			} else {
	...
	*/
	function deleteObject( $table, &$object, $keyName ) {
		...
		$this->setQuery( sprintf( $fmtsql, $where ) );
		return $this->query();
	}

	/**
	* @param boolean If TRUE, displays the last SQL statement sent to the database
	* @return string A standardized error message
	*/
